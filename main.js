// ==========================================
// DYNAMIC LAB CONTENT MATRIX DATABASE
// ==========================================
const definitionDb = [
...
// --- DOM Bootstrap Routing ---
window.addEventListener('load', async () => {
    if (document.getElementById('definitions-grid')) renderDefinitionsGrid();
    if (document.getElementById('fc-card')) initializeFlashcardSystem();
    if (document.getElementById('practice-chapter-filter')) {
        populatePracticeNavigationDropdowns();
        loadSelectedPracticeProblem();
    }
    
    // Globally establish the Python VM environments
...
async function bootstrapPyodideVirtualMachine() {
    ...
    try {
        ...
        statusTxt.innerText = "Loading scientific packages (numpy, sympy)...";
        await pyodideInstance.loadPackage(['numpy', 'sympy']);
        await pyodideInstance.runPythonAsync("import numpy as np; import sympy as sp");
        banner.classList.add('loaded');
        if (spinner) spinner.style.display = "none";
        statusTxt.innerHTML = "✨ Python 3.13 Environment fully online (Pyodide Matrix Sandbox ready!)";
...
// --- Definitions Grid logic ---
function renderDefinitionsGrid(filteredItems = definitionDb) {
    ...
    filteredItems.forEach(item => {
        ...
        card.innerHTML = `
            ...
            <div class="def-example-box"><code>${escapeHtml(item.example)}</code></div>
        `;
...
function applyFilters() {
    ...
    const results = definitionDb.filter(item => {
...
// --- Flashcards Component engine ---
function initializeFlashcardSystem() {
    flashcardDeck = [...definitionDb];
...
function updateFlashcardDisplay() {
    ...
    setTimeout(() => {
        ...
        document.getElementById('fc-tag').innerText = item.chapTitle;
        document.getElementById('fc-term').innerText = item.func;
        document.getElementById('fc-definition').innerText = item.desc;
        document.getElementById('fc-code').innerText = item.example;
    }, 180);
}
...
function populatePracticeNavigationDropdowns() {
    ...
    practiceWorkouts.forEach((workout, index) => {
...
function loadSelectedPracticeProblem() {
    const index = document.getElementById('practice-chapter-filter').value;
    const workout = practiceWorkouts[index];
    if (!workout) return;

    document.getElementById('instruction-badge').innerText = workout.badge;
    document.getElementById('instruction-title').innerText = workout.title;
    document.getElementById('instruction-body').innerHTML = workout.instructions;
    document.getElementById('instruction-ideal').innerText = workout.expected;
    document.getElementById('practice-editor').value = workout.starter;
    document.getElementById('practice-terminal').innerText = "Console ready.";
}

function handlePracticeChapterChange() { loadSelectedPracticeProblem(); }
function resetPracticeBoilerplate() { loadSelectedPracticeProblem(); }
function resetSandboxCode() {
    document.getElementById('sandbox-editor').value = "# Sandbox Cleared.\n";
}

// --- Compiling VM execution logic ---
async function executePythonRun(editorId, terminalId) {
    ...
    try {
        ...
        if (stderr) {
            ...
            terminal.innerText = stdout || "Code completed with no standard outputs.";
        }
    } catch (err) {
        terminal.className = 'terminal-box error';
